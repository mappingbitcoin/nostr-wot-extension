...
function createWeblnApi() {
    ...
    return { webln, pending, postedMessages, simulateResponse, forceEnable };
}

describe('WebLN API shape', () => {
    it('has enabled property defaulting to false', () => {
        const { webln } = createWeblnApi();
...
describe('WebLN enable gate', () => {
    ...
    it('getInfo does not require enable', () => {
        const { webln, postedMessages } = createWeblnApi();
    ...
    it('sendPayment works after enable', () => {
        const { webln, forceEnable, postedMessages } = createWeblnApi();
        forceEnable();
...
describe('WebLN response routing', () => {
    it('resolves promise on matching WEBLN_RESPONSE', async () => {
        const { webln, postedMessages, simulateResponse } = createWeblnApi();
        const promise = webln.getInfo();
    ...
    it('ignores response for unknown id', () => {
        const { simulateResponse, pending } = createWeblnApi();
        // Should not throw
    ...
    it('cleans up pending entry after response', async () => {
        const { webln, postedMessages, simulateResponse, pending } = createWeblnApi();
        const promise = webln.getInfo();
